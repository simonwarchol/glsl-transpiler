    ...
    if (this.optimize && descriptor.optimize !== false && descriptor.components) {
        ...
        if (complexity < descriptor.complexity) {
            ...
            if (descriptor.components && descriptor.components.length > 1) {
                var include = descriptor.components.map(function (c) {
                    return c.include;
                }, this).filter(Boolean)
...
GLSL.prototype.transforms = {
    ...
    continue: function () {
        return Descriptor('continue')
    },

    break: function () {
        return Descriptor('break')
    },
    ...
    group: function (node) {
        ...
        return Descriptor(result, {
            ...
            complexity: children.reduce(function (prev, curr) {
                return prev + curr.complexity || 0
            }, 0)
        })
    }
...
GLSL.prototype.unswizzle = function (node) {
    ...
    //vec2 a.xy → a
    if (type && args.length === this.types[type].length && positions.every(function (position, i) {
        return position === i
    })) {
        return ident
    }
